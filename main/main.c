...
//#define ws2811_commit(X, Y) ws2811_commit_bitbang(X, Y, PB5)
#define ws2811_commit(X, Y) ws2811_commit_pwm(X, Y)
//#define ws2811_commit(X, Y) send(X, Y, PB5)
//#define ws2811_commit(X, Y) SPI_MasterTransmit(0xa4)
...
#define WS2811_LED_STEP (0xFFFFUL / WS2811_LED_COUNT)
    ...
    if (now >= before_ws2811 + 100) {
        ...
        ws2811_commit(&data[0], 3 * 71);
        ws2811_commit(&data[71], 3 * 71);
    }
